...
class MultipleParamListTests extends AnyFlatSpec with matchers.should.Matchers {
  ...
  it should "work with client/server simulation" in {
    ...
    val server = com.goodcover.tagless.TwoWaySimulator.server(impl)
    val client = com.goodcover.tagless.TwoWaySimulator.client[MultipleParamListTestAlg, Id](server)
