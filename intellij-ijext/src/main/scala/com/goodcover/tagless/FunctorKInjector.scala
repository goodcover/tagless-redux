...
object FunctorKInjector {
  ...
  private[this] val kryoEncoderAnn  = "com.goodcover.tagless.kryo.kryoEncoder"
  private[this] val pekkoEncoderAnn = "com.goodcover.tagless.pekko.pekkoEncoder"
  ...
  private def mkWireProtocolKryo(clazz: ScTypeDefinition) = if (isKryoEncoder(clazz)) {
    ...
    typeParams(clazz).toSeq.flatMap { case (tpName, tpText) =>
      Seq(
        s"implicit def taglessWireProtocol${clazz.name}$tpName: _root_.com.goodcover.tagless.util.WireProtocol[$tpText] = ???"
      )
    }
  ...
  private def mkWireProtocolPekko(clazz: ScTypeDefinition) = if (isPekkoEncoder(clazz)) {
    ...
    typeParams(clazz).toSeq.flatMap { case (tpName, tpText) =>
      Seq(
        s"implicit def taglessWireProtocol${clazz.name}$tpName(implicit system: _root_.org.apache.pekko.actor.ActorSystem): _root_.com.goodcover.tagless.util.WireProtocol[$tpText] = ???"
      )
    }
