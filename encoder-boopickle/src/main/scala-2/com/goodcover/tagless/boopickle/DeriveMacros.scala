        ...
        case method @ Method(name, _, _, TypeRef(_, _, outParams), _) =>
          ...
          val body =
            q"""(_root_.boopickle.Default.Pickle.intoBytes((${name.name.toString}, ..$args)).array()
                ,_root_.com.goodcover.tagless.boopickle.BoopickleCodec.decoder[${outParams.last}]
                )"""
            ...
            case (acc, Method(name, _, pss, TypeRef(_, _, outParams), _)) =>
              ...
              val pair =
                q"""
                    ...
                    val resultEncoder = _root_.com.goodcover.tagless.boopickle.BoopickleCodec.encoder[$out]
                    _root_.com.goodcover.tagless.util.PairE(invocation, resultEncoder)
                  """
